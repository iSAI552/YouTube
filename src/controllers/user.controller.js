...
const generateAccessTokenAndRefreshToken = async (userId) => { // created a general function that can be used again & again 
...
const registerUser = asyncHandler( async (req, res) => {
   ...
   // check for images, check compulsorily for avatar
...
const getUserChannelPofile = asyncHandler( async (req, res) => {
    ...
    const channel = await User.aggregate([
        ...
        {
            $lookup: {
                from: "subscriptions", // The Schema name was Subscription but when the model is saved in MongoDB it makes
                //                          it in lowercase and plural
                localField: "_id",
        ...
        },
        {
            $addFields: {
                ...
                isSubscribed: {
                    $cond: {
                        if: {$in: [req.user?._id, "$subscribers.subsbcriber"]},
                        then: true,
                        else: false
                    }
                }
